...
function updateStandings() {
  ...
  const sortedTeams = Object.entries(teams).sort((a, b) => {
    return (
      b[1].points - a[1].points || b[1].goalDifference - a[1].goalDifference
    );
  });
  ...
  sortedTeams.forEach(([teamName, stats]) => {
    ...
    row.insertCell(1).textContent = stats.points;
    row.insertCell(2).textContent = stats.matches;
    row.insertCell(3).textContent = stats.wins;
    row.insertCell(4).textContent = stats.draws;
    row.insertCell(5).textContent = stats.losses;
    row.insertCell(6).textContent = stats.goalDifference;
  });
}
